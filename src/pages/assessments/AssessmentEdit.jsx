...
const AssessmentEdit = () => {
  ...
  const handleSubmit = async () => {
    if (!title || sections.some(s => !s.title || s.questions.some(q => !q.text))) {
      alert("Please fill all titles and question texts/options.");
      return;
    }

    try {
        console.log(title,sections)
      const data=await axios.put(`/assessments/${assessmentId}`, { title, sections });
      ...
      <div className="flex flex-col w-[48vw]">
        ...
        >
          ...
          <input
            ...
            className={`border rounded p-2 w-full mb-4 text-lg font-semibold ${
            ...
            <div key={sIdx} className="mb-6 relative">
              ...
              <input
                ...
                className={`border rounded p-1 w-full mb-3 font-semibold text-lg ${
                  theme === "dark" ? "text-white bg-gray-800 border-gray-600" : "text-black bg-white border-gray-300"
                }`}
              />
        ...
        <button
          className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 w-60 ml-auto mt-2"
              ...
              <div key={qIdx} className="mb-4">
                ...
                {q.type === "single-choice" &&
                  q.options.map((opt, idx) => (
                    <div key={idx}><input type="radio" name={`q${sIdx}_${qIdx}`} /> {opt}</div>
                  ))}

                {q.type === "multi-choice" &&
                  q.options.map((opt, idx) => (
                    <div key={idx}><input type="checkbox" /> {opt}</div>
                  ))}

                {q.type === "short-text" && <input type="text" className="border rounded p-1 w-full" />}
                {q.type === "long-text" && <textarea className="border rounded p-1 w-full" rows={3} />}
                {q.type === "numeric" && <input type="number" className="border rounded p-1 w-full" />}
              </div>
            ))}
